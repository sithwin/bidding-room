...
**Goal:** Build `@carat-room/shared-logger` — a shared pino-based structured logger with `AsyncLocalStorage` request context. Every service imports this package to get per-request `requestId` and `correlationId` bound to every log line automatically, without threading a logger through every function signature.
...
- No `any` — strict mode throughout
- Package name: `@carat-room/shared-logger`
