...
describe('GET /api/auctions', () => {
  it('should_return200WithAuctionsAndLotCounts', async () => {
    ...
    const body = await res.json() as { data: Array<{ id: string; title: string; saleDate: string; lotCount: number }> };
  ...
  it('should_passStatusFilterAndLimit_when_provided', async () => {
    ...
    const res = await app.request('/api/auctions?status=upcoming&limit=3');
