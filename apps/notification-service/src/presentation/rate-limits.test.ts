...
import { buildNotificationRateLimits } from './rate-limits';
...
describe('buildNotificationRateLimits', () => {
  let redis: Redis;
  ...
  it('should_returnTooManyRequests_when_defaultLimitExceeded', async () => {
    ...
    const { default: defaultLimiter } = buildNotificationRateLimits(redis);
    const app = new Hono();
    app.use('*', defaultLimiter);
    app.get('/health-probe', (c) => c.json({ data: { ok: true } }));

    const res = await app.request('/health-probe', {
